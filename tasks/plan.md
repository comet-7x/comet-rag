...
本计划记录 Comet-RAG 如何从“6600 行零测试的引擎集合”推进到“一条可部署、
有回归保护的 DOCX 入库与检索链路”。
...
4. **高风险任务前置**
   M1 开始时，`docx_parser.py`（962 行零测试）和 Milvus 写入可见性语义是两个
   最可能翻车的点，因此分别安排在 T13、T21，并在各自阶段早期完成。
...
- [X] T3 — 修复当时无法直接运行的 `docs/pipeline_usage.md` 示例
...
- [X] `poc/task_demo/` 已删除；`poc/` 被 `.gitignore` 覆盖，不属于版本库交付物
...
- [X] **抽象评审完成**：Phase 4/5 在未修改业务契约的前提下替换真实后端并通过
  同一套契约、集成与崩溃恢复测试
...
| R2 | M1 开始时 **`docx_parser.py` 962 行零测试**，改动可能无声破坏解析质量 | 高 | T13 已补快照测试；M1 内未做无保护重构 |
