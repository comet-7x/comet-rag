...
- [ ] T4 — 把 `poc/task_demo/demo.py` 的 5 个场景转成 pytest
...
- [ ] T22 — `ArqExecutor`（必须通过 T8 的同一套契约测试）
- [ ] T23 — `workers/preprocessor.py` 与 `workers/embedder.py`
- [ ] T24 — `sweep_stale` 定时器与崩溃恢复

**Checkpoint E**
- [ ] `kill -9` 掉 worker，任务在租约超时内退回 PENDING 并被另一 worker 接管完成
- [ ] preprocessor 用多进程扩容、embedder 用单进程高并发（不得反过来）
