...
def _first_extension(label: str, extensions: list[str]) -> str:
    return extensions[0] if extensions else label


def detect_content_type_from_bytes(content: bytes) -> str:
    """输入二进制内容，返回标准文件扩展名"""
    result = get_magika().identify_bytes(content)
    return _first_extension(result.output.label, result.output.extensions)
