...
down_revision: Union[str, Sequence[str], None] = 'e008a15e0906'
...
def upgrade() -> None:
    ...
    sa.Column('request', sa.JSON().with_variant(postgresql.JSONB(astext_type=sa.Text()), 'postgresql'), nullable=True),
    ...
    sa.UniqueConstraint('kind', 'idempotency_key', name='uq_tasks_kind_idempotency')
    )
    op.create_index(op.f('ix_tasks_created_at'), 'tasks', ['created_at'], unique=False)
    op.create_index('ix_tasks_kind_created', 'tasks', ['kind', 'created_at'], unique=False)
    op.create_index(op.f('ix_tasks_owner_id'), 'tasks', ['owner_id'], unique=False)
    op.create_index(op.f('ix_tasks_status'), 'tasks', ['status'], unique=False)
    op.create_index('ix_tasks_status_heartbeat', 'tasks', ['status', 'heartbeat_at'], unique=False)
    ...
    sa.ForeignKeyConstraint(['task_id'], ['tasks.task_id'], name=op.f('fk_task_events_task_id_tasks'), ondelete='CASCADE'),
